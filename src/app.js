...
let processingStats = { pending: 0, capacity: 3, wasmMode: "starting", workerTimeMs: null, sourceLatencyMs: null, processedBlocks: 0, sequenceGaps: 0 };
...
function bindProcessing() {
  processing.addEventListener("spectrum", (event) => {
    ...
    sourceStats.levelDbfs = detail.levelDbfs;
    processingStats = {
      ...processingStats,
      wasmMode: detail.wasmMode,
      workerTimeMs: detail.workerTimeMs,
      sourceLatencyMs: detail.sourceLatencyMs,
      processedBlocks: detail.processedBlocks,
      sequenceGaps: detail.sequenceGaps
    };
...
function renderReceiver() {
  staticView(`<section class="view">
    ...
    <div class="receiver-layout">
      ...
      <div class="receiver-side">
        <section class="instrument-panel framebuffer-panel"><header class="panel-header"><span class="panel-title">Mayhem 240 × 320 port target</span></header><div class="framebuffer-shell"><canvas id="mayhemCanvas" aria-label="Mayhem logical framebuffer port status"></canvas></div><div class="port-target-note">The authentic upstream framebuffer is not linked in this development batch. This canvas states that limitation instead of imitating completed Mayhem behavior.</div></section>
        <div class="quick-controls"><button id="quickStart" class="primary-button" type="button">Start Receiver</button><button id="quickStop" class="secondary-button" type="button">Stop Receiver</button><button id="quickCapture" class="secondary-button" type="button">Start Capture</button><button id="quickStation" class="secondary-button" type="button">Save Station</button></div>
      </div>
    </div>
  </section>`);

  spectrumView = new SpectrumWaterfallView({ spectrumCanvas: $("spectrumCanvas"), waterfallCanvas: $("waterfallCanvas") });
  spectrumView.configure({ ...currentSettings(), markers: projectStore.project.markers, tuningStepHz: getTuningStep() });
  ...
  $("clearPeak").addEventListener("click", () => { spectrumView.clearPeak(); processing?.updateSettings({ peakHold: currentSettings().peakHold }, true); });
...
function renderStations() {
  ...
  if (!projectStore.project.stations.length) host.append(node("div", { class: "empty-state" }, node("div", node("strong", { text: "No saved stations" }), node("p", { text: "Tune a frequency and save it here. Fresh Start does not load demo presets." }), node("button", { class: "primary-button", type: "button", text: "Save current station", onclick: saveCurrentStation }))));
    ...
    for (const station of projectStore.project.stations) {
      ...
      body.append(node("tr", {},
        node("td", { text: station.name }), node("td", { text: formatFrequency(station.frequencyHz, 3) }), node("td", { text: formatRate(station.sampleRate) }),
...
function exportStationsCsv() {
  const rows = [["name", "frequency_hz", "sample_rate", "gain_db", "ppm", "notes"]];
  for (const station of projectStore.project.stations) rows.push([station.name, station.frequencyHz, station.sampleRate, station.gainDb ?? "automatic", station.ppm ?? 0, station.notes ?? ""]);
...
function exportDiagnostics() {
  ...
  const payload = createDiagnosticPackage({
    preflight, browser, connection: { state: stateMachine.state, history: stateMachine.history }, device: radio.safeDeviceInfo(includeSerial), receiver: effectiveActual(), stream: activeStreamStats(), processing: processingStats, capture: captureStore?.activeStatus, application, project: projectStore.project, logs: log.toJSON(), includeSerial
  });
...
function renderHelp() {
  staticView(`<section class="view">${pageHeading("HELP", "Connect, inspect, capture, and recover", "The essential receive workflow is available without configuring transfer depth, ring capacity, direct sampling, or bias-tee power.")}
    ...
    <article class="card"><span class="eyebrow">INPUT</span><h2>Keyboard and pointer controls</h2><p><span class="kbd">←</span> <span class="kbd">→</span> tune by the selected step. Hold Shift for ten times the step.</p><p>Click the spectrum to tune, Shift-click to add a marker, drag to pan, and use the wheel to zoom.</p><p>The authentic 240 × 320 Mayhem input mapping is reserved for the upstream framebuffer integration batch.</p></article></div>
...
async function startSource(options = {}) {
  ...
  try {
    if (sourceType === "live") {
      await radio.startReceiver({ blockSamples: currentSettings().usbBlockSamples, transferDepth: currentSettings().transferDepth, onBlock: consumeBlock });
    } else if (sourceType === "simulation") {
      simulation.configure({ sampleRate: currentSettings().sampleRate, centerFrequencyHz: currentSettings().centerFrequencyHz, blockSamples: currentSettings().usbBlockSamples });
      ...
      replay.start(consumeBlock, { speed: options.replaySpeed ?? 1, blockSamples: currentSettings().usbBlockSamples });
    ...
    log.info("Sample source started", { sourceType });
...
function resetCurrentInspector() {
  if (currentView === "receiver") {
    ...
    processing?.updateSettings(currentSettings(), true);
    spectrumView?.configure(currentSettings());
  }
...
async function initialize() {
  ...
  try {
    processing = new ProcessingClient({ workerUrl: new URL("./workers/processing-worker.js", import.meta.url), wasmUrl: new URL("../assets/dsp_core.wasm", import.meta.url), settings: currentSettings(), log, maxPendingBlocks: 3 });
    bindProcessing();
